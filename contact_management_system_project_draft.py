...
contacts = {"1234567890": {"Name": "John Smith", "Email": "johnsmith@gmail.com"}}
...
def add_new_contact():
    ...
    while True:
        try:
            ...
            number = new_contact_number.group().replace('(', '').replace('-', '').replace(')', '').replace(' ', '')
            print(number)
            ...
            new_contact.update({number: {"Name": new_contact_name, "Email": new_contact_email.group()}}) # Updates new_contact with the new number including the .group() method so it appears readable. It also adds the new name and new email address (using .group()) to their indicated keys in our contacts dicitonary.
...
def search_for_contact(): # need help with this
    while True:
        try:
            ...
            search_results = re.match(r"\(?\d{3}\)?(\s|-)?\d{3}(\s|-)?\d{4}", contact_search)
            search_results = search_results.group().replace('(', '').replace('-', '').replace(')', '').replace(' ', '')
            print(search_results)
            if search_results in contacts:
                print(contacts[search_results])
                break
            else:
                print("Contact not found")
                continue
